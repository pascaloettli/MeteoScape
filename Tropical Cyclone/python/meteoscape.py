...
def Hodge_decomposition(
    ...
    # )
    x = np.sum(np.nan_to_num(adata.obsm[vel_2d_key_]/np.linalg.norm(adata.obsm[vel_2d_key_],axis=1)[:,np.newaxis]) * adata.obsm[pot_vkey_],axis=1)
    norm2_grad = np.linalg.norm(np.max(x,0))
    x = np.sum((adata.obsm[vel_2d_key_]/np.linalg.norm(adata.obsm[vel_2d_key_],axis=1)[:,np.newaxis]) * adata.obsm[rot_vkey_],axis=1)
    norm2_curl = np.linalg.norm(np.max(np.nan_to_num(x),0))
    log_["Contribution_ratio"]["Potential"] = "{:.2%}".format(
      norm2_grad / (norm2_grad + norm2_curl)
    )
    log_["Contribution_ratio"]["Rotation"] = "{:.2%}".format(
      norm2_curl / (norm2_grad + norm2_curl)
    )
...
def view(
    ...
    **kwargs,
    ...
    if show_graph:
        ...
        ax.tripcolor(
            tri_, adata.obs[color_key], lw=0.5, zorder=0, alpha=alpha, cmap=kwargs["cmap"]
        )
    if cluster_key != None:
...
def view_surface(
    ...
    save=False,
    save_dir=None,
    save_filename="CellMap_view_surface",
    figsize=None,
    fontsize_text=16,
    cbar=True,
    **kwargs,
...
def view_stream_line(
    ...
    **kwargs,
    ...
    titles = ["2D velocity orbit", "Development orbit", "Periodic orbit"]
...
def view_surface_3D(
    ...
    save=False,
    save_dir=None,
    save_filename="CellMap_surface_3D",
    **kwargs,
):
    kwargs_arg = _check_arguments(
        adata,
...
def view_trajectory(
    ...
    ax.triplot(tri_, color="gray", zorder=0, alpha=alpha, lw=2)
...
def dynamical_clustering(
    ...
    cluster_key="clusters",
    streamfunc_key="streamfunc",
    titles=["Histogram", "Dynamical clustering"],
    palette=['#1F77B4FF', '#FF7F0EFF'],
    figsize=(10, 8),
    fs_label = 16,
    fs_lagened = 16,
    ...
    for i_trg_ in range(n_clusters_):
        ...
        ax1.hist(adata.obs[sl_key_][idx_],bins=bins,lw=1,edgecolor="k",label=target_clusters[i_trg_],color=palette[i_trg_])
        ...
        ax2.plot(x_range, pdf_values,lw=3,zorder=10,color=palette[i_trg_])
    ax1.set_ylabel('Frequency',fontsize=fs_label)
    ...
    if save_png:
        filename = (
            ...
            if save_dir == None
            else "%s/%s_Gaussian" % (save_dir, save_filename)
        )
        fig.savefig(filename + ".png", bbox_inches="tight")
    if save_svg:
        filename = (
    ...
    for i in range(max(idx_color_)+1):
        idx_ = idx_color_ == i
        # ax.scatter(adata.obsm[basis_key][idx_,0],adata.obsm[basis_key][idx_,1],s=marker_size,c=cmap(i))
        ax.scatter(adata.obsm[basis_key][idx_,0],adata.obsm[basis_key][idx_,1],s=marker_size,c=palette[i])
    for i_trg_ in range(n_clusters_):
        cluster_ = target_clusters[i_trg_]
        idx_ = adata.obs[cluster_key] == cluster_
        # ax.scatter(adata.obsm[basis_key][idx_,0],adata.obsm[basis_key][idx_,1],s=marker_size*4,c=cmap(i_trg_),edgecolors="k")
        ax.scatter(adata.obsm[basis_key][idx_,0],adata.obsm[basis_key][idx_,1],s=marker_size*4,c=palette[i_trg_],edgecolors="k")
        txt = ax.text(np.mean(adata.obsm[basis_key][idx_,0]),np.mean(adata.obsm[basis_key][idx_,1]),target_clusters[i_trg_],fontsize=fsize,ha="center", va="center",fontweight="bold", zorder=20,)
        txt.set_path_effects([PathEffects.withStroke(linewidth=5, foreground="w")])
    ax.axis("off")
    ax.margins(0.05, 0.05)
    ax.set_title(titles[1], fontsize=40)   
    if save_png:
        filename = (
            "%s_clusters" % (save_filename)
            if save_dir == None
            else "%s/%s_clusters" % (save_dir, save_filename)
        )
        fig.savefig(filename + ".png", bbox_inches="tight")
    if save_svg:
        filename = (
